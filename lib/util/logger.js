...
if (process.env.NODE_ENV === 'production') {
  ...
  winston.loggers.options.transports.push(new WinstonDailyRotateFile(fileOptions));
}

winston.loggers.add('default').filters.push((level, msg) => `[default] ${msg}`);
...
module.exports = function getLogger(filename) {
  ...
  const name = path.relative(CONFIG.ROOT_PATH, filename);
  if (!winston.loggers.has(name)) {
    winston.loggers.add(name).filters.push((level, msg) => `[${name}] ${msg}`);
  }
